...
export default function Register() {
  ...
  function validateName(value: string): string | undefined {
    if (!value) return undefined; // required attr handles empty-on-submit
  ...
  function validateContact(value: string): string | undefined {
    if (!value) return undefined; // contact is optional until submit-time full check
    if (!isValidPhone(value)) return "Enter a valid 10-digit mobile number.";
    return undefined;
  }
  ...
  function handleContactChange(value: string) {
    // allow only digits while typing, capped at 10
  ...
  function validateAllOnSubmit(): boolean {
    ...
    // NOTE: contact is treated as optional — only validated if the user entered something.
    // If contact should be mandatory, add `required` to the TextInput below and drop the
    // `form.contact.length > 0 &&` guard here.
    const contactError =
